...
class DatasetProvider(abc.ABC):
    ...
    def dataset_processor(instruments_d, column_names, start_time, end_time, freq, inst_processors=[]):
        ...
        if len(new_data) > 0:
            # 检查所有 DataFrame 的列是否一致，避免广播错误
            all_columns = set()
            for inst in new_data.keys():
                all_columns.update(new_data[inst].columns)
            
            # 确保所有 DataFrame 都有相同的列（缺失的列填充 NaN）
            # 注意：inst_calculator 返回的列名是 normalize_column_names（normalize后的）
            # 所以这里应该使用 normalize_column_names 进行比较
            expected_columns = set(normalize_column_names)
            for inst in new_data.keys():
                # 如果DataFrame为空（没有行），跳过这个股票，不填充NaN
                if len(new_data[inst]) == 0:
                    continue
                
                missing_cols = expected_columns - set(new_data[inst].columns)
                if missing_cols:
                    for col in missing_cols:
                        if col in expected_columns:
                            # 创建一个与现有数据相同长度的NaN数组
                            new_data[inst][col] = np.nan
                
                # 只保留 normalize_column_names 中存在的列，并按 normalize_column_names 的顺序排列
                available_cols = [col for col in normalize_column_names if col in new_data[inst].columns]
                if available_cols:
                    new_data[inst] = new_data[inst][available_cols]
                else:
                    # 如果没有任何列，但DataFrame不为空，创建NaN列
                    if len(new_data[inst]) > 0:
                        new_data[inst] = pd.DataFrame(index=new_data[inst].index, columns=normalize_column_names)
                    # 如果DataFrame为空，保持原样（不填充NaN）
            
            # 过滤掉空DataFrame
            new_data = {inst: df for inst, df in new_data.items() if len(df) > 0}
            
            data = pd.concat(new_data, names=["instrument"], sort=False)
            # 确保列的顺序与 normalize_column_names 一致
            # 注意：data的列名是 normalize_column_names（normalize后的）
            available_cols = [col for col in normalize_column_names if col in data.columns]
            if available_cols:
                data = data[available_cols]
            # 如果列数不匹配，说明有列缺失，需要处理
            if len(data.columns) != len(normalize_column_names):
                # 检查是否有列名不匹配的情况
                missing_cols = set(normalize_column_names) - set(data.columns)
                if missing_cols:
                    # 为缺失的列创建NaN列
                    for col in missing_cols:
                        data[col] = np.nan
                # 重新排序列（使用normalize后的列名）
                data = data[normalize_column_names]
            # 最后调用 cache_to_origin_data 将 normalize 后的列名转换回原始 column_names
    ...
    def inst_calculator(inst, start_time, end_time, freq, column_names, spans=None, g_config=None, inst_processors=[]):
        ...
        obj = dict()
        # 先尝试加载所有字段，找到第一个有数据的字段作为基准
        base_series = None
        for field in column_names:
            try:
                series = ExpressionD.expression(inst, field, start_time, end_time, freq)
            ...
            for field in column_names:
                ...
                try:
                    ...
                    if series is not None and hasattr(series, '__len__') and len(series) > 0:
                        ...
                        # 空数据，创建NaN数组
                        obj[field] = pd.Series(np.nan, index=base_index, name=field)
                except Exception as e:
                    # 计算失败，创建NaN数组
                    get_module_logger("data").debug(f"Failed to calculate {field} for {inst}: {e}")
                    obj[field] = pd.Series(np.nan, index=base_index, name=field)
        else:
            # 如果所有字段都加载失败，返回空DataFrame（让上层处理）
            return pd.DataFrame(index=pd.DatetimeIndex([], name="datetime"))

        # 确保所有 Series 的索引一致（只在真正需要时重新索引）
        if len(obj) > 1:
            # 找到最长的索引作为基准
            max_len = 0
            base_index = None
            for series in obj.values():
                if hasattr(series, '__len__') and len(series) > max_len:
                    max_len = len(series)
                    base_index = series.index
            
            # 如果找到了基准索引，确保所有 Series 都使用相同的索引
            if base_index is not None:
                for field in list(obj.keys()):
                    series = obj[field]
                    # 只在索引长度不同或索引不相等时才重新索引
                    if hasattr(series, 'index'):
                        if len(series.index) != len(base_index):
                            # 重新索引，缺失的值填充 NaN
                            obj[field] = series.reindex(base_index)
                        elif not series.index.equals(base_index):
                            # 索引长度相同但内容不同，尝试对齐
                            try:
                                obj[field] = series.reindex(base_index)
                            except:
