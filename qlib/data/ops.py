...
try:
    ...
    # Fallback to Python implementations if Cython extensions are not available
    print("⚠️ Warning: Cython extensions not available, using Python fallback implementations")
    # Simple Python fallback implementations
    def rolling_slope(data, window, min_periods=None):
        """Python fallback for rolling_slope"""
        return data.rolling(window=window, min_periods=min_periods).apply(
            lambda x: np.polyfit(range(len(x)), x, 1)[0] if len(x) >= 2 else np.nan, raw=True
        )
    
    def rolling_rsquare(data, window, min_periods=None):
        """Python fallback for rolling_rsquare"""
        return data.rolling(window=window, min_periods=min_periods).apply(
            lambda x: np.corrcoef(range(len(x)), x)[0,1]**2 if len(x) >= 2 else np.nan, raw=True
        )
    
    def rolling_resi(data, window, min_periods=None):
        """Python fallback for rolling_resi"""
        return data.rolling(window=window, min_periods=min_periods).apply(
            lambda x: np.polyfit(range(len(x)), x, 1)[1] if len(x) >= 2 else np.nan, raw=True
        )
    
    def expanding_slope(data, min_periods=None):
        """Python fallback for expanding_slope"""
        return data.expanding(min_periods=min_periods).apply(
            lambda x: np.polyfit(range(len(x)), x, 1)[0] if len(x) >= 2 else np.nan, raw=True
        )
    
    def expanding_rsquare(data, min_periods=None):
        """Python fallback for expanding_rsquare"""
        return data.expanding(min_periods=min_periods).apply(
            lambda x: np.corrcoef(range(len(x)), x)[0,1]**2 if len(x) >= 2 else np.nan, raw=True
        )
    
    def expanding_resi(data, min_periods=None):
        """Python fallback for expanding_resi"""
        return data.expanding(min_periods=min_periods).apply(
            lambda x: np.polyfit(range(len(x)), x, 1)[1] if len(x) >= 2 else np.nan, raw=True
        )
except ValueError:
