...
class DatasetCache(BaseProviderCache):
    ...
    def cache_to_origin_data(data, fields):
        ...
        # 检查实际数据的列名格式
        # 如果列名是tuple格式（如 ('feature', '$open')），需要提取第二个元素进行匹配
        if len(data.columns) > 0:
            first_col = data.columns[0]
            if isinstance(first_col, tuple) and len(first_col) == 2:
                # 列名是tuple格式，提取第二个元素进行匹配
                actual_col_names = [col[1] if isinstance(col, tuple) and len(col) == 2 else str(col) for col in data.columns]
                # 创建映射：not_space_fields -> 实际列名
                col_mapping = {}
                for field, not_space_field in zip(fields, not_space_fields):
                    # 尝试匹配
                    matching_cols = [col for col, name in zip(data.columns, actual_col_names) if name == not_space_field]
                    if matching_cols:
                        col_mapping[not_space_field] = matching_cols[0]
                    else:
                        # 如果找不到匹配，使用第一个匹配的列（如果有）
                        # 或者创建NaN列
                        col_mapping[not_space_field] = None
                
                # 重新构建DataFrame
                new_data = pd.DataFrame(index=data.index)
                for field, not_space_field in zip(fields, not_space_fields):
                    if col_mapping[not_space_field] is not None:
                        new_data[not_space_field] = data[col_mapping[not_space_field]]
                    else:
                        # 创建NaN列
                        new_data[not_space_field] = np.nan
                
                data = new_data
            else:
                # 列名是字符串格式，直接匹配
                existing_cols = [col for col in not_space_fields if col in data.columns]
                missing_cols = [col for col in not_space_fields if col not in data.columns]
