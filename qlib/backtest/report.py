...
class PortfolioMetrics:
    ...
    def _cal_benchmark(benchmark_config: Optional[dict], freq: str) -> Optional[pd.Series]:
        ...
        if isinstance(benchmark, pd.Series):
            ...
            # 调试信息：打印基准查询参数
            print(f"DEBUG: _cal_benchmark query: codes={_codes}, start_time={start_time}, end_time={end_time}, freq={freq}")
            _temp_result, _ = get_higher_eq_freq_feature(_codes, fields, start_time, end_time, freq=freq)
            print(f"DEBUG: _cal_benchmark result: shape={_temp_result.shape}, empty={_temp_result.empty}")
            # 如果结果为空且freq是day，尝试使用1min频率查询（适用于高频数据）
            if len(_temp_result) == 0 and freq in ["day", "1d"]:
                print(f"DEBUG: _cal_benchmark retry with 1min freq")
                _temp_result, _ = get_higher_eq_freq_feature(_codes, fields, start_time, end_time, freq="1min")
                print(f"DEBUG: _cal_benchmark retry result: shape={_temp_result.shape}, empty={_temp_result.empty}")
