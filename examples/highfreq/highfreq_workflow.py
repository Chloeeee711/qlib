...
cache_candidates = [
    ...
    "/mnt/d/qlib_cache",
    "/data/qlib_cache",
    "/home/intern0/.cache/qlib",
]
for candidate in cache_candidates:
    try:
        os.makedirs(candidate, exist_ok=True)
        test_file = Path(candidate) / "test.tmp"
...
os.environ.setdefault("JOBLIB_MAX_NBYTES", "1M")
...
for candidate in tmp_dir_candidates:
    try:
        os.makedirs(candidate, exist_ok=True)
...
FEATURES_DIR = Path(KQ_DATA_DIR) / "features"
...
if FEATURES_DIR.exists():
    ...
    for folder, final_path in rename_list:
        try:
            ...
            print(f"✅ 重命名: {folder.name} -> {final_path.name}")
            rename_count += 1
        except PermissionError as e:
    ...
    # 创建小写符号链接，解决 FileFeatureStorage instrument.lower() 问题
    ...
    for folder in FEATURES_DIR.iterdir():
        ...
        if lowercase_path.exists():
            if lowercase_path.is_symlink():
                try:
                    if lowercase_path.resolve() == folder.resolve():
                        symlink_skipped += 1
                        continue
                ...
                continue
        try:
            lowercase_path.symlink_to(folder.name)
            symlink_created += 1
            if symlink_created <= 5:
                print(f"✅ 创建符号链接: {lowercase_name} -> {folder.name}")
        except Exception as e:
            print(f"❌ 创建符号链接失败 {lowercase_name}: {e}")
            symlink_error += 1
    print(f"📊 符号链接修复完成: 创建 {symlink_created} 个, 跳过 {symlink_skipped} 个, 失败 {symlink_error} 个")
else:
...
STOCK_POOL_CSV = "/home/intern0/qlib/examples/highfreq/sorted_high_preclose_ratio_2025.csv"
...
if isinstance(C.provider_uri, str):
    ...
    print(f"🔍 当前 provider_uri: {C.provider_uri}")
else:
    print(f"⚠️ provider_uri 类型异常: {type(C.provider_uri)}")
    try:
        C.provider_uri = QlibConfig.DataPathManager.format_provider_uri(C.provider_uri)
        print(f"🔧 provider_uri 已转换为字典格式: {C.provider_uri}")
    except Exception as e:
        print(f"❌ provider_uri 无法转换: {e}")

actual_provider_path = C.provider_uri
if isinstance(C.provider_uri, dict):
    actual_provider_path = C.provider_uri.get("__DEFAULT_FREQ", C.provider_uri)

print(f"🔍 验证：当前 Qlib 数据路径: {actual_provider_path}")
if str(actual_provider_path) != KQ_DATA_DIR:
    print(f"⚠️ 警告：数据路径不匹配！")
    print(f"  期望路径: {KQ_DATA_DIR}")
    print(f"  实际路径: {actual_provider_path}")
    print("💡 建议检查天勤数据转换是否完整")
else:
    print("✅ 数据路径匹配正确")

TEST_STOCK_COUNT = int(os.environ.get("TEST_STOCK_COUNT", "0"))

stock_pool_df = pd.read_csv(STOCK_POOL_CSV)
stock_codes_raw = stock_pool_df["code"].tolist()
print(f"📊 股票池包含 {len(stock_codes_raw)} 行（原始）")

stock_codes_unique = stock_pool_df["code"].drop_duplicates().tolist()
if len(stock_codes_unique) != len(stock_codes_raw):
    print(f"   ⚠️ 发现重复股票，去重后 {len(stock_codes_unique)} 支")
else:
    print(f"   ✅ 无重复股票，合计 {len(stock_codes_unique)} 支")
...
candidate_codes = [convert_code_format(code) for code in stock_codes_unique]
...
if features_root.exists():
    available_codes = sorted(
        [d.name.upper() for d in features_root.iterdir() if d.is_dir()]
    )
else:
    ...
    if instruments_file.exists():
        with open(instruments_file, "r", encoding="utf-8") as f:
            available_codes = [
                line.split("\t")[0].strip().upper() for line in f if line.strip()
            ]

if not available_codes:
    raise FileNotFoundError(f"无法在 {KQ_DATA_DIR} 找到可用股票目录或 instruments/all.txt")

available_set = set(available_codes)
missing_codes = [code for code in candidate_codes if code not in available_set]
if missing_codes:
    print(f"⚠️ 有 {len(missing_codes)} 支股票在数据目录中不存在，示例: {missing_codes[:5]}")

candidate_codes = [code for code in candidate_codes if code in available_set]
print(f"📊 匹配结果: CSV {len(stock_codes_unique)} 支，数据目录 {len(available_codes)} 支，匹配 {len(candidate_codes)} 支")

if not candidate_codes:
    raise ValueError("数据目录中不存在与股票池匹配的股票，请检查数据准备。")

if TEST_STOCK_COUNT > 0 and len(candidate_codes) > TEST_STOCK_COUNT:
    candidate_codes = candidate_codes[:TEST_STOCK_COUNT]
    print(f"🧪 测试模式：仅保留前 {len(candidate_codes)} 支（目标 {TEST_STOCK_COUNT}）")
else:
    print(f"📌 使用全部匹配股票: {len(candidate_codes)} 支")
...
USE_MARKET_FILTER = os.environ.get("USE_MARKET_FILTER", "1").lower() in ("1", "true", "yes")
...
if market_needs_refresh:
    if not USE_MARKET_FILTER:
        ...
        print(f"⏭️ 跳过覆盖率筛选，直接使用候选股票: {len(stock_codes)} 支")
    else:
        stock_codes = candidate_codes
        print("⏳ filtered_market.pkl 不存在或强制重建，将在后续步骤执行覆盖率筛选")
else:
    with open(FILTERED_MARKET_PATH, "rb") as f:
        stock_codes = pickle.load(f)
    original_cache_size = len(stock_codes)
    stock_codes = [code for code in stock_codes if code in available_set]
    if len(stock_codes) != original_cache_size:
        print(f"⚠️ 缓存股票池有 {original_cache_size - len(stock_codes)} 支缺失，已剔除")
    if not stock_codes:
        print("⚠️ 缓存为空，自动触发重新筛选")
        stock_codes = candidate_codes
        market_needs_refresh = True

print(f"📌 当前股票池大小: {len(stock_codes)}")
...
if market_needs_refresh:
    if USE_MARKET_FILTER:
        ...
        FILTERED_MARKET_PATH.write_bytes(pickle.dumps(stock_codes))
        print(f'⏭️ 跳过覆盖率筛选，使用全部 {len(stock_codes)} 支候选股票')
        print(f'✅ 股票池已保存 -> {FILTERED_MARKET_PATH}')
else:
...
handler_cfg['kwargs']['instruments'] = stock_codes
dataset_task['kwargs']['handler'] = handler_cfg
...
class WorkingLowFreqStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        topk=50,
        signal=None,
        signal_threshold=0.0005,  # 降低阈值，让更多信号通过
        n_drop=3,  
        rebalance_ratio=0.2,
                ...
                if isinstance(self.signal.index, pd.MultiIndex):
                    ...
                    if date_mask.any():
                        ...
                        if time_mask_14_40.any():
                            ...
                            print(f"   阈值以上信号: {len(pred_score[pred_score >= 0.0005])}")
                        else:
...
def calculate_equal_weight_overnight_benchmark(
    ...
    """
    使用与标签完全相同的方式计算隔夜收益，只在 10:46 时间点记录收益。
    """
    from qlib.data import D
    ...
    for inst in stock_codes:
        try:
            if isinstance(all_data.index, pd.MultiIndex):
                inst_levels = all_data.index.get_level_values("instrument").unique()
...
def predict_and_save_csv():
    ...
    # 2. 读取股票池
    stock_pool_df = pd.read_csv(Path("/home/intern0/qlib/examples/highfreq/sorted_high_preclose_ratio_2025.csv"))
