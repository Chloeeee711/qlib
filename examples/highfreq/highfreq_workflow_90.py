...
def download_historical_data():
    ...
    RAW_DIR = PROJECT_ROOT.parent / "kq_raw_data_recent"
    QLIB_DIR = PROJECT_ROOT.parent / "qlib_data_recent"
...
# ====== 天勤数据目录 - 历史数据路径（用于训练） ======
KQ_DATA_DIR = Path(r"E:\qlib_data_new")  # 历史数据根目录
RAW_DIR = Path(r"E:\kq_raw_data_new")  # 历史原始数据
...
# 转换股票代码格式（all.txt需要匹配文件夹格式：SSE_600036）
# 注意：由于文件夹名称是 SSE_600036 格式，而 qlib 会使用 instrument.lower() 查找文件夹
# 所以 all.txt 中应该使用 SSE_600036 格式，而不是 SH600036 格式
def convert_to_folder_format(code):
    """将各种格式转换为文件夹格式（SSE_600036 或 SZSE_300757）"""
    code_upper = code.upper()
    if code_upper.startswith('SH'):
        return f"SSE_{code_upper[2:]}"
    elif code_upper.startswith('SZ'):
        return f"SZSE_{code_upper[2:]}"
    elif code_upper.startswith('SSE_') or code_upper.startswith('SZSE_'):
        return code_upper
    elif '.' in code_upper:
...
inst_df["instrument"] = inst_df["instrument"].apply(convert_to_folder_format)
...
print(f"📊 股票池原始代码: {len(stock_codes_raw)} 支股票")

# 转换代码格式：600036.SH -> SSE_600036, 300757.SZ -> SZSE_300757
# 注意：由于文件夹名称是 SSE_600036 格式，而 qlib 会使用 instrument.lower() 查找文件夹
# 所以需要将代码转换为 SSE_600036 格式，匹配文件夹名称
# 同时，all.txt 中也应该使用 SSE_600036 格式
def convert_code_format(code):
    """将 600036.SH 格式转换为 SSE_600036 格式（匹配数据文件夹格式）"""
    if '.' in code:
        parts = code.split('.')
        if len(parts) == 2:
            num, market = parts
            if market.upper() == 'SH':
                return f"SSE_{num}"  # 使用SSE_格式，匹配文件夹名称
            elif market.upper() == 'SZ':
                return f"SZSE_{num}"  # 使用SZSE_格式，匹配文件夹名称
    # 如果已经是正确格式，检查是否需要转换
    code_upper = code.upper()
    if code_upper.startswith('SH') and not code_upper.startswith('SSE_'):
        # SH600036 -> SSE_600036
        return f"SSE_{code_upper[2:]}"
    elif code_upper.startswith('SZ') and not code_upper.startswith('SZSE_'):
        # SZ300757 -> SZSE_300757
        return f"SZSE_{code_upper[2:]}"
    # 如果已经是SSE_/SZSE_格式，直接返回
    if code_upper.startswith('SSE_') or code_upper.startswith('SZSE_'):
        return code_upper
    return code_upper

stock_codes = [convert_code_format(code) for code in stock_codes_raw]
print(f"📊 股票池转换后: {len(stock_codes)} 支股票")
print(f"📊 前5个代码示例: {stock_codes[:5]}")
...
# 检查数据文件的实际时间范围
# 从第一个股票的数据文件读取start_index，确定实际的数据时间范围
from qlib.data.data import Cal
import numpy as np
cal = Cal.calendar(freq='1min')

# 尝试从第一个股票的数据文件读取start_index
test_code = stock_codes[0] if len(stock_codes) > 0 else 'SSE_600036'
test_file = Path(f"E:/qlib_data_new/features/{test_code}/close.1min.bin")
if test_file.exists():
    data = np.fromfile(test_file, dtype='<f', count=1)
    data_start_idx = int(data[0]) if len(data) > 0 else 0
    print(f"📅 从 {test_code} 数据文件读取到 start_index: {data_start_idx}")
    if data_start_idx < len(cal):
        actual_start_time = cal[data_start_idx]
        # 查询从实际开始时间往后5分钟
        check_end_idx = min(data_start_idx + 5, len(cal) - 1)
        actual_end_time = cal[check_end_idx]
        start_time = str(actual_start_time)
        end_time = str(actual_end_time)
        print(f"📅 数据文件实际时间范围: {start_time} ~ {end_time}")
    else:
        # 如果索引超出范围，使用默认时间
        start_time = "2025-01-02 09:30:00"
        end_time = "2025-01-03 15:00:00"
        print(f"⚠️ start_index超出日历范围，使用默认时间范围")
else:
    # 如果文件不存在，使用默认时间
    start_time = "2025-01-02 09:30:00"
    end_time = "2025-01-03 15:00:00"
    print(f"⚠️ 测试文件不存在，使用默认时间范围")

fields = ["$close"]

# 只检查stock_codes中的股票，而不是all.txt中的所有股票
# 对每个股票单独检查其实际的时间范围（从数据文件的start_index开始）
print(f"📊 开始检查股票池中的 {len(stock_codes)} 支股票是否有数据...")
ok_codes, bad_codes = [], []

for idx, code in enumerate(stock_codes):  # 使用转换后的stock_codes，而不是all.txt中的所有codes
    try:
        # 对每个股票，从数据文件读取start_index，确定实际的时间范围
        test_file = Path(f"E:/qlib_data_new/features/{code}/close.1min.bin")
        if test_file.exists():
            data = np.fromfile(test_file, dtype='<f', count=1)
            code_start_idx = int(data[0]) if len(data) > 0 else None
            if code_start_idx is not None and code_start_idx < len(cal):
                code_start_time = cal[code_start_idx]
                code_end_idx = min(code_start_idx + 5, len(cal) - 1)
                code_end_time = cal[code_end_idx]
                code_start_str = str(code_start_time)
                code_end_str = str(code_end_time)
            else:
                # 如果无法读取start_index，使用默认时间范围
                code_start_str = start_time
                code_end_str = end_time
        else:
            # 如果文件不存在，使用默认时间范围
            code_start_str = start_time
            code_end_str = end_time
        
        df = D.features([code], fields, start_time=code_start_str, end_time=code_end_str, freq="1min")
        if df.empty:
            bad_codes.append(code)
            if idx < 5:  # 只打印前5个失败的
                print(f"  ⚠️ {code}: 数据为空 (检查时间: {code_start_str} ~ {code_end_str})")
        else:
            ok_codes.append(code)
            if idx < 5:  # 只打印前5个成功的
                print(f"  ✅ {code}: 有数据 ({len(df)} 条记录, 时间: {code_start_str} ~ {code_end_str})")
    except Exception as e:
        bad_codes.append(code)
        if idx < 5:  # 只打印前5个失败的
            print(f"  ❌ {code} 检查失败: {str(e)[:100]}")
...
# 使用筛选后的股票列表（ok_codes），而不是从pickle重新读取
# 这样可以确保使用的是转换后的代码格式
if len(ok_codes) > 0:
    stock_codes = ok_codes  # 直接使用ok_codes，确保格式正确
    print(f"✅ 使用筛选后的股票列表: {len(stock_codes)} 支股票")
    print(f"前10个代码: {stock_codes[:10]}")
else:
    print(f"⚠️ 警告: 数据检查阶段没有找到有数据的股票！")
    print(f"   这可能是因为:")
    print(f"   1. 数据还未转换完成")
    print(f"   2. 时间范围不匹配")
    print(f"   3. 数据路径不正确")
    print(f"   4. 股票代码格式不匹配（all.txt格式与文件夹格式不一致）")
    print(f"   将使用原始转换后的代码继续运行（共 {len(stock_codes)} 支）")
    print(f"   前10个代码: {stock_codes[:10]}")
    # stock_codes 已经是转换后的格式，保持不变
...
if xtrain.size == 0 or xtest.size == 0:
    ...
    # 检查特征值范围
    if train_nan_count < xtrain.size * 0.5:  # 如果NaN值不超过50%
    ...
    if isinstance(ytrain, pd.DataFrame):
        label_col = ytrain.columns[0]
        label_data = ytrain[label_col]
...
class WorkingLowFreqStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        topk=50,
        signal=None,
        signal_threshold=0.005,  # 降低阈值，让更多信号通过
        n_drop=3,  
        rebalance_ratio=0.2,
...
port_analysis_config_minute = {
    ...
    "backtest": {
        ...
        "end_time": None,
        "account": 1e9,
        "benchmark": "SSE_000300",  # 使用SSE_000300格式，匹配数据文件夹格式
        "exchange_kwargs": {
            "freq": "1min",
            "deal_price": "close",  
...
print(f"✅ 回测end_time已设置: {next_min} (类型: {type(next_min)})")
...
try:
    ...
    if 'bench' in report.columns:
        bench_col = report["bench"]
    else:
        bench_col = pd.Series(0.0, index=report.index)
    
    er_wo_cost = risk_analysis(report["return"] - bench_col)
    er_w_cost = risk_analysis(report["return"] - bench_col - report["cost"])
    print("超额（无成本）:\n", er_wo_cost.round(4).to_string())
    print("\n超额（含成本）:\n", er_w_cost.round(4).to_string())
except Exception as e:
    print(f"⚠️ 计算超额收益时出错: {e}")
...
# 如果基准为None，bench列可能不存在或全为0/NaN，需要处理
if 'bench' in report.columns:
    bench_values = report['bench'].fillna(0.0)  # 将NaN填充为0
else:
    bench_values = pd.Series(0.0, index=report.index)  # 如果没有bench列，创建全0的Series

arr_wo_cost = (1 + (report['return'] - bench_values)).to_numpy(dtype='float32')
arr_w_cost = (1 + (report['return'] - bench_values - report['cost'])).to_numpy(dtype='float32')
arr_bench = (1 + bench_values).to_numpy(dtype='float32')
...
# 如果没有基准，cum_bench 为空Series
if 'arr_bench' in locals() and arr_bench is not None and len(arr_bench) > 0:
    cum_bench = pd.Series(block_cumprod(arr_bench, chunk=5000), index=report.index)
else:
    cum_bench = pd.Series(dtype=float, index=report.index)  # 空Series
...
def check_data_integrity():
    ...
    # 检查几个关键股票的数据（使用实际存在的股票代码）
    test_codes = ['SSE_600036', 'SSE_600519', 'SZSE_000001']  # 使用SSE_/SZSE_格式
