...
def _detect_incremental_start(calendar_file: Path, default_start: str) -> str:
    """根据已有calendar推断增量下载起点（返回YYYY-MM-DD字符串）"""
    try:
...
def download_historical_data():
    ...
    RAW_DIR = Path("/home/intern0/kq_raw_data_ipynb")   # 与训练流程保持一致
    QLIB_DIR = Path("/home/intern0/qlib_data_ipynb")    # 直接追加到历史数据目录
    POOL_CSV = Path("/home/intern0/qlib/examples/highfreq/sorted_high_preclose_ratio_2025.csv")
    ...
    if INCREMENTAL_DOWNLOAD:
        calendar_file = QLIB_DIR / "calendars" / "1min.txt"
        START = _detect_incremental_start(calendar_file, START_DEFAULT)
        print(f"🔁 增量模式：从 {START} 之后追加新数据（不覆盖既有历史）")
    else:
        START = START_DEFAULT
    ...
    if QLIB_ROOT not in existing_pythonpath.split(":"):
        env["PYTHONPATH"] = f"{QLIB_ROOT}:{existing_pythonpath}" if existing_pythonpath else QLIB_ROOT
    success = False
    for attempt in range(1, MAX_DOWNLOAD_RETRIES + 1):
        print(f"🔁 下载任务尝试 {attempt}/{MAX_DOWNLOAD_RETRIES}")
...
# ====== 天勤数据目录 - 历史数据路径（用于训练） ======
KQ_DATA_DIR = Path("/home/intern0/qlib_data_ipynb")  # 历史数据根目录
RAW_DIR = Path("/home/intern0/kq_raw_data_ipynb")  # 历史原始数据
...
FEATURES_DIR = Path("/home/intern0/qlib_data_ipynb/features")  # 使用历史数据路径
...
OUTPUT_FILE_SUFFIX = "_alpha158fz7_optimized"  # 此脚本的输出文件后缀（优化版本）
...
# 天勤数据配置
KQ_DATA_DIR = "/home/intern0/qlib_data_ipynb"
STOCK_POOL_CSV = "/home/intern0/qlib/examples/highfreq/sorted_high_preclose_ratio_2025.csv"  # 改为500股票池
...
TEST_STOCK_COUNT = int(os.environ.get("TEST_STOCK_COUNT", "500"))

stock_pool_df = pd.read_csv(STOCK_POOL_CSV)
stock_codes_raw = stock_pool_df["code"].tolist()
print(f"📊 股票池包含 {len(stock_codes_raw)} 支股票（原始格式）")
print(f"   前5个原始代码: {stock_codes_raw[:5]}")
...
candidate_codes = [convert_code_format(code) for code in stock_codes_raw]
...
FILTERED_MARKET_PATH = Path("filtered_market.pkl")
...
if market_needs_refresh:
    ...
    print("⏳ filtered_market.pkl 不存在或强制重建，使用候选股票集合")
else:
...
# ====== 模型超参数配置 ======
# 是否启用超参数优化
ENABLE_HYPEROPT = os.environ.get("ENABLE_HYPEROPT", "0").lower() in ("1", "true", "yes")
HYPEROPT_METHOD = os.environ.get("HYPEROPT_METHOD", "grid")  # grid, random, bayesian
HYPEROPT_TRIALS = int(os.environ.get("HYPEROPT_TRIALS", "20"))  # 优化轮数

# 默认参数（如果不用优化）
_use_gpu = str(os.environ.get("USE_GPU", "0")).lower() in ("1", "true", "yes")
_default_lgbm_kwargs = {
    "loss": "mse",
    "learning_rate": 0.1,
    "n_estimators": 100,
    "num_leaves": 31,
...
# 如果启用优化，将在训练前调用优化器
if ENABLE_HYPEROPT:
    _lgbm_kwargs = None  # 将在优化后确定
    print(f"🔍 超参数优化已启用: 方法={HYPEROPT_METHOD}, 轮数={HYPEROPT_TRIALS}")
else:
    _lgbm_kwargs = _default_lgbm_kwargs.copy()
    print("⚙️ 使用默认模型参数（设置 ENABLE_HYPEROPT=1 可启用超参数优化）")

model_task = {
    "class": "LGBModel",
    "module_path": "qlib.contrib.model.gbdt",
    "kwargs": _lgbm_kwargs if _lgbm_kwargs is not None else _default_lgbm_kwargs,
}
...
# ====== 超参数优化（如果启用） ======
if ENABLE_HYPEROPT and _lgbm_kwargs is None:
    print("\n" + "="*70)
    print("🔍 开始超参数优化")
    print("="*70)
    print(f"   优化方法: {HYPEROPT_METHOD}")
    print(f"   优化轮数: {HYPEROPT_TRIALS}")
    
    try:
        from model_optimizer import ModelHyperparameterOptimizer
        
        # 创建优化器（使用测试集作为验证集）
        optimizer = ModelHyperparameterOptimizer(
            dataset_train=dataset_train,
            dataset_val=dataset_clean,  # 使用测试集作为验证集
            metric='ic',  # 优化目标：IC
            output_suffix=OUTPUT_FILE_SUFFIX
        )
        
        # 执行优化
        if HYPEROPT_METHOD == "grid":
            ...
            best_params = optimizer.random_search(
                ...
                n_iter=HYPEROPT_TRIALS
            )
        elif HYPEROPT_METHOD == "bayesian":
            best_params = optimizer.bayesian_optimization(n_trials=HYPEROPT_TRIALS)
        else:
            print(f"⚠️ 未知的优化方法: {HYPEROPT_METHOD}，使用默认参数")
            best_params = _default_lgbm_kwargs.copy()
        
        # 更新模型参数
        _lgbm_kwargs = _default_lgbm_kwargs.copy()
        _lgbm_kwargs.update(best_params)
        
        # 更新 task 配置
        task["model"]["kwargs"] = _lgbm_kwargs
        ...
        # 保存最优参数
        import json
        best_params_path = Path(f"best_hyperparams{OUTPUT_FILE_SUFFIX}.json")
        ...
        if optimizer.optimization_history:
            ...
            opt_history_path = Path(f"hyperopt_history{OUTPUT_FILE_SUFFIX}.csv")
            opt_history_df.to_csv(opt_history_path, index=False, encoding="utf-8-sig")
            print(f"💾 优化历史已保存: {opt_history_path}")
...
print(f"   模型参数: {task['model']['kwargs']}")
    ...
    for inst in stock_codes:
        try:
            if isinstance(all_data.index, pd.MultiIndex):
                instruments = all_data.index.get_level_values("instrument").unique()
                if inst not in instruments:
                    continue
                df = all_data.xs(inst, level="instrument", drop_level=True)
            else:
                if inst not in all_data.index:
                    continue
                df = all_data.loc[inst]
            ...
            morning_next_day = morning_vwap.shift(-1)
...
try:
    ...
    if not risk_metrics.empty:
        ...
        # 保存风险指标（可选）
        SAVE_RISK_CSV = os.environ.get("SAVE_RISK_CSV", "0").lower() in ("1", "true", "yes")
        if SAVE_RISK_CSV:
            risk_metrics_path = Path(f"risk_metrics{OUTPUT_FILE_SUFFIX}.csv")
            risk_metrics.to_csv(risk_metrics_path, index=False, encoding="utf-8-sig")
            print(f"\n💾 风险指标已保存: {risk_metrics_path}")
    
    # 稳定性分析
    print("\n" + "="*70)
    print("📉 稳定性分析（滚动窗口）")
    print("="*70)
    stability_metrics = analyzer.calculate_stability_metrics(window_days=30)
    if not stability_metrics.empty:
        print(stability_metrics.to_string(index=False))
        
        # 稳定性指标不保存CSV（数据量大）
        print("   💡 稳定性指标仅打印，不保存CSV")
    ...
    if not attribution.empty:
        print(attribution.head(20).to_string(index=False))  # 只显示前20条
        if len(attribution) > 20:
            print(f"   ... 还有 {len(attribution) - 20} 条记录")
        
        # 保存归因分析（可选）
        SAVE_ATTRIBUTION_CSV = os.environ.get("SAVE_ATTRIBUTION_CSV", "0").lower() in ("1", "true", "yes")
        if SAVE_ATTRIBUTION_CSV:
            attribution_path = Path(f"attribution_analysis{OUTPUT_FILE_SUFFIX}.csv")
            attribution.to_csv(attribution_path, index=False, encoding="utf-8-sig")
            print(f"\n💾 归因分析已保存: {attribution_path}")
    ...
    try:
        ...
        if not trade_metrics.empty:
            print(trade_metrics.to_string(index=False))
            
            # 交易分析不保存CSV（可选）
            print("   💡 交易分析仅打印，不保存CSV")
    except Exception as e:
        print(f"⚠️ 交易分析失败: {e}")
    ...
    try:
        ...
        print("✅ 分析结果已保存到 MLflow recorder")
    except Exception as e:
        print(f"⚠️ 保存到 MLflow 失败: {e}")
