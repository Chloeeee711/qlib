...
if '/home/intern0/qlib' not in sys.path:
    sys.path.insert(0, '/home/intern0/qlib')
...
FEATURES_DIR = Path("/home/intern0/qlib_data_tail_end/features")  # 使用历史数据路径
...
import importlib
if 'highfreq_handler_alpha158' in sys.modules:
    importlib.reload(sys.modules['highfreq_handler_alpha158'])
    print("🔄 已重新加载 highfreq_handler_alpha158 模块")
...
STOCK_POOL_CSV = "/home/intern0/qlib/examples/highfreq/tail_end_candidates.csv"  # 改为500股票池

# 天勤数据时间段配置
START_TIME = "2025-01-02 09:30:00"
END_TIME = "2025-11-13 14:57:00"  # 修改：测试集截止到09-29，14:57避免回测越界
...
# 注意：对于高频数据，provider_uri 可以是字符串路径，qlib 会自动转换为字典格式
# 但为了确保兼容性，我们也可以直接使用字典格式
CUSTOM_QLIB_CONFIG = {
    "provider_uri": KQ_DATA_DIR,  # 字符串格式，qlib 会自动转换为 {'__DEFAULT_FREQ': KQ_DATA_DIR}
    "dataset_cache": None,
...
# 重要：不要覆盖 provider_uri，qlib 会自动将其转换为字典格式
# 如果强制设置为字符串会导致 'str' object has no attribute 'keys' 错误
from qlib.config import C, QlibConfig
# 检查 provider_uri 的格式，如果是字符串则转换为字典
...
if isinstance(C.provider_uri, dict):
    ...
    if str(actual_path) != KQ_DATA_DIR:
        ...
        print(f"✅ 数据路径匹配正确")
elif isinstance(C.provider_uri, str):
    print(f"🔍 验证：当前 Qlib 数据路径（字符串格式）: {C.provider_uri}")
    if C.provider_uri != KQ_DATA_DIR:
        print(f"⚠️ 警告：数据路径不匹配！")
        print(f"  期望路径: {KQ_DATA_DIR}")
    ...
    print(f"🔍 验证：当前 Qlib 数据路径: {C.provider_uri} (类型: {type(C.provider_uri)})")
...
# 转换股票代码格式：sh.603072 -> SH603072, sz.000001 -> SZ000001
def convert_code_format(code):
    ...
    if '.' in code:
        ...
        if len(parts) == 2:
            ...
            stock_code = parts[1]  # 603072
            if market == 'SH' or market == 'SSE':
                return f"SH{stock_code}"
            elif market == 'SZ' or market == 'SZSE':
...
print(f"   转换后前5个代码: {stock_codes[:5]}")

if TEST_STOCK_COUNT > 0:
    stock_codes = stock_codes[:TEST_STOCK_COUNT]
    print(f"🧪 测试模式：仅保留前 {len(stock_codes)} 支股票（目标 {TEST_STOCK_COUNT}）")
...
# 读取 instruments 文件
instruments_file = Path("/home/intern0/qlib_data_ipynb/instruments/all.txt")
...
# 注意：数据文件格式要求：
# 1. 数据文件第一个值必须是 start_index（整数，通常是0）
# 2. 文件夹名称需要小写（已通过符号链接解决）
# 如果数据读取失败，请检查数据文件格式
...
for code in codes:
    try:
        ...
        if df.empty:
            ...
            if test_count < max_test:
                print(f"✅ 有数据 (形状: {df.shape})")
        test_count += 1
        if test_count >= max_test and len(ok_codes) == 0:
            print(f"⚠️ 前 {max_test} 个股票都没有数据")
            print(f"   可能原因：")
            print(f"   1. 数据文件格式不正确（第一行是0占位符）")
            print(f"   2. 数据文件时间索引不匹配")
            print(f"   3. 需要运行数据修复脚本: python3 /home/intern0/qlib/fix_data_files.py")
            break
    except Exception as e:
        bad_codes.append(code)
        if test_count < max_test:
...
if TEST_STOCK_COUNT > 0 and len(stock_codes) > TEST_STOCK_COUNT:
    stock_codes = stock_codes[:TEST_STOCK_COUNT]
    print(f"🧪 测试模式：筛选后仅保留前 {len(stock_codes)} 支股票（目标 {TEST_STOCK_COUNT}）")
...
import importlib
import os
handler_module_path = os.path.join(WORKDIR, "highfreq_handler_alpha158.py")
...
dataset_task = {
    ...
    "kwargs": {
        "handler": {
            ...
            "module_path": "highfreq_handler_alpha158",  # 确保使用 alpha158 版本
...
print(f"   期望的模块: highfreq_handler_alpha158")

# 尝试直接导入验证
try:
    import highfreq_handler_alpha158 as hf_alpha158
    print(f"✅ 成功导入 highfreq_handler_alpha158")
    ...
    print(f"   ✅ get_feature_config() 返回特征数量: {len(test_cfg['feature'][0])}")
    if len(test_cfg['feature'][0]) == 173:
        print(f"   ✅ 特征数量正确（173个）")
    else:
        print(f"   ⚠️ 特征数量不正确，期望173个，实际{len(test_cfg['feature'][0])}个")
        print(f"   前20个特征名称: {test_cfg['feature'][1][:20]}")
except Exception as e:
    print(f"❌ 导入或测试 highfreq_handler_alpha158 失败: {e}")
    import traceback
    traceback.print_exc()
