...
class HighFreqHandler(DataHandlerLP):
    ...
    def get_feature_config(self):
        ...
        # 分钟化适配：将 Alpha158 的“日窗口/日滞后/日级 vwap”转为分钟级表达式
        def _minuteize_expr(expr: str, minutes_per_day: int = 240) -> str:
            import re
            e = expr
            # 1) 替换 $vwap 为分钟级可用表达式（使用与基础特征一致的 simpson_vwap）
            e = e.replace("$vwap", simpson_vwap)

            # 2) 将具有窗口参数的常见函数的“日窗口 → 分钟窗口”
            # 这些函数的最后一个参数通常是窗口长度（除了 Ref 是第2个参数）
            window_funcs_last = [
                "Mean","Std","Var","Skew","Kurt","Mad","Max","Min","Sum","Prod",
                "Quantile","Rank","DecayLinear","DecayExp","Rolling","ArgMax","ArgMin",
                "Corr","Cov"
            ]
            for fn in window_funcs_last:
                # 仅替换最后一个“纯数字”窗口参数
                # 例: Mean(x, 20) -> Mean(x, 4800)
                pattern = rf"{fn}\s*\(([^()]*)\)"
                def repl(m):
                    args = m.group(1)
                    ...
                    if len(parts) >= 1:
                        try:
                            ...
                            if re.fullmatch(r"\d+", last):
                                parts[-1] = str(int(last) * minutes_per_day)
            ...
            # 3) Ref 的第二个参数是滞后长度（天数）；映射为分钟级
            # 例: Ref($close, 1) -> Ref($close, 240)
            pattern_ref = r"Ref\s*\(([^(),]+)\s*,\s*(-?\d+)\s*\)"
            def repl_ref(m):
                a1 = m.group(1).strip()
                a2 = m.group(2).strip()
                try:
                    return f"Ref({a1}, {int(a2) * minutes_per_day})"
                except Exception:
                    return m.group(0)
            e = re.sub(pattern_ref, repl_ref, e)

            return e

        minute_alpha_fields = []
        for af in alpha158_fields:
            try:
                mfe = _minuteize_expr(str(af))
                # 统一用分钟级 Cut 打包，避免边界泄露
                minute_alpha_fields.append(f"Cut({mfe}, 240, None)")
            except Exception:
                # 容错：若分钟化失败，退回原表达式
                minute_alpha_fields.append(f"Cut({af}, 240, None)")

        fields += minute_alpha_fields
        names += alpha158_names
        print(f"[Alpha158] 分钟化后特征数量: {len(minute_alpha_fields)}")
        print(f"[Alpha158] 加载后特征数量: {len(fields)}")

        # ========== 添加 FactorZoo 扩展因子 ==========
        factor_zoo_features = [
            ("FZ_MAX_HIGH_MIN_LOW_RATIO", "Max($high, 240) / Min($low, 240) - 1"),
            ("FZ_MAD_HIGH_CLOSE_RATIO", "Mad($high, 240) / $close"),
            ("FZ_STD_CLOSE_RET_4", "Std($close / Ref($close, 4) - 1, 240)"),
            ("FZ_KURT_CLOSE_RET_4", "Kurt($close / Ref($close, 4) - 1, 240)"),
            ("FZ_CORR_HIGH_VOLUME", "Corr(Ref($high, 1), $volume, 237)"),
            ("FZ_MAX_CLOSE", "Max($close, 240)"),
            ("FZ_MIN_CLOSE_RET_7", "Min($close / Ref($close, 7) - 1, 240)"),
        ]

        print(f"[FactorZoo] 开始追加自定义因子: {len(factor_zoo_features)} 个")
        for name, expr in factor_zoo_features:
            fields.append(expr)
            names.append(name)

        print(f"[FactorZoo] 追加后特征总数: {len(fields)}")
...
from highfreq_handler import HighFreqHandler as VanillaHighFreqHandler  # 仅用于安全兜底
import logging
...
class SafeHighFreqHandler(VanillaHighFreqHandler):
